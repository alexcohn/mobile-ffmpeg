...
JNIEXPORT int JNICALL Java_com_arthenica_mobileffmpeg_Config_runTranscode(JNIEnv *env, jclass object, jstring inFilename, jstring outFilename)
{
...
cleanup:
    ...
    (*env)->ReleaseStringUTFChars(env, inFilename, in_filename);
    (*env)->ReleaseStringUTFChars(env, outFilename, out_filename);
    return ret;
}
